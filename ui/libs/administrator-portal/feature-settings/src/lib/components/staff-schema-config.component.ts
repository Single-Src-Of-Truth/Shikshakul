...
@Component({
  selector: 'lib-staff-schema-config',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './staff-schema-config.component.html',
  styleUrl: './staff-schema-config.component.scss',
})
export class StaffSchemaConfigComponent implements OnInit {
  ...
  initForm() {
    this.schemaForm = this.fb.group({
      schema_name: ['Staff Onboarding Schema', Validators.required],
  ...
  loadSchema() {
    this.loading = true;
    this.teacherService.getActiveSchema().subscribe({
      next: (res) => {
  ...
  saveSchema() {
    ...
    this.saving = true;
    this.teacherService.createSchema(this.schemaForm.value).subscribe({
      next: () => {
  ...
  deleteSchema() {
    if (confirm('Are you sure you want to delete the active schema? All dynamic staff fields will be lost.')) {
      this.saving = true;
      this.teacherService.deleteSchema().subscribe({
        next: () => {
